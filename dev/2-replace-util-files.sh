...
find_cmd="find . -type f \( $fileTypes \) -exec grep '$oldProjName' {} +"
echo "Executing: $find_cmd"
eval "${find_cmd}"

# mac os
#find . -type f \( -name '*.toc' -o -name '*.xml' \) -exec sed -i '' "s/AddonTemplate/$newProjName/g" {} \;
cmd="find . -type f \( $fileTypes \) -exec sed -i '' \"s/$oldProjName/$newProjName/g\" {} \;"
# all others
# cmd="find . -type f \( $fileTypes \) -exec sed -i \"s/AddonTemplate/$newProjName/g\" {} \;"
